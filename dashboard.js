...
import { myDrivingRides, myJoinedRides } from '../../../shared/rides.js';
import { myIncomingRequests, myOutgoingRequests, respondToRequest } from '../../../shared/requests.js';
import { GUARDIAN_STATUS_LABELS, REQUEST_STATUS_LABELS } from '../../../shared/constants.js';
import { whenLine, relativeTime } from '../../../shared/format.js';
...
async function load() {
  ...
  $('#welcome').innerHTML = `
    <h1 style="font-size:1.9rem;margin-bottom:.2rem">Welcome, ${esc((profile?.full_name || 'there').split(' ')[0])}</h1>
    <p class="muted small mb-0">Here's what's happening with your rides.</p>`;
  ...
  /* ------------------------------------------------------------- stats -- */
  const tile = (icon, label, value, note, href) => `
    <a class="card card-hover" href="${href}" style="text-decoration:none;color:inherit">
      <div class="tiny muted" style="text-transform:uppercase;letter-spacing:.06em">${icon} ${esc(label)}</div>
      <div style="font-size:1.9rem;font-weight:800;line-height:1.25">${esc(String(value))}</div>
      <div class="tiny muted">${esc(note)}</div>
    </a>`;

  $('#stats').innerHTML =
    tile('🗓️', 'Upcoming rides', upcomingDriving.length + upcomingJoining.length,
         `${upcomingDriving.length} driving · ${upcomingJoining.length} joining`, 'my-rides.html') +
    tile('💺', 'Seats available', openSeats,
         openSeats ? 'across the rides you\'re driving' : 'no open seats right now', 'my-rides.html') +
    ...
    tile('✉️', 'Your pending requests', pendingOut.length,
