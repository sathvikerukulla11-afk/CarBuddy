...
-- -------------------------------------------------------------- utilities --
-- Marks the current transaction as running privileged (system) logic.
-- Only SECURITY DEFINER functions defined in this schema call it, so client
-- code can never flip the flag and bypass the column guards below.
...
begin
  if not public.is_privileged() then
    ...
    if new.age_category is distinct from old.age_category
       and old.age_category <> 'adult'
       and new.age_category = 'adult' then
...
create trigger trg_touch_profiles_private
...
set search_path = public, pg_temp
as $$
  select coalesce((select is_suspended from public.profiles where id = p_uid), false);
$$;
...
set search_path = public, pg_temp
as $$
  select coalesce((select verification_status = 'verified' from public.profiles where id = p_uid), false);
$$;
