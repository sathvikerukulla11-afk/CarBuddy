...
create trigger trg_guard_ride before update on public.rides
...
create trigger trg_guard_ride_insert before insert on public.rides
...
create trigger trg_touch_ride_meetups before update on public.ride_meetups
...
create unique index if not exists ride_requests_one_active
  on public.ride_requests(ride_id, rider_id) where status in ('pending', 'accepted');
...
as $$ select exists (select 1 from public.rides where id = p_ride and driver_id = p_uid); $$;

create or replace function public.is_ride_participant(p_ride uuid, p_uid uuid default auth.uid())
returns boolean language sql stable security definer set search_path = public, pg_temp
as $$
  select exists (select 1 from public.ride_participants
...
returns boolean language sql stable security definer set search_path = public, pg_temp
as $$
  select exists (select 1 from public.ride_requests where ride_id = p_ride and rider_id = p_uid);
$$;
...
-- of which lock the ride row. This lets an admin verify integrity.
create or replace function public.recount_ride_seats(p_ride uuid)
returns smallint language plpgsql security definer set search_path = public, pg_temp
as $$
