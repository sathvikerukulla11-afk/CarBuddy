...
create trigger trg_group_creator
  after insert on public.trusted_groups
...
create unique index if not exists guardian_rel_unique_active
  on public.guardian_relationships(guardian_id, minor_id)
  where status = 'active';
...
-- A member is allowed to act (post/join rides) when they are not suspended and,
-- if they are a minor, a guardian is linked to their account.
