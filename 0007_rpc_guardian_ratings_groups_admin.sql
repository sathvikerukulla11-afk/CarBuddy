...
begin
  ...
  select invite_code into v_code
   ...
   where minor_id = v_me and status = 'pending'
...
create or replace function public.claim_guardian_invite(p_code text, p_relationship text default 'Parent/Guardian')
...
begin
  ...
  select * into v_rel
    from public.guardian_relationships
   where upper(invite_code) = upper(trim(p_code)) and status = 'pending'
   for update;

  if not found then
    raise exception 'That code is not valid or has already been used' using errcode = 'P0002';
  end if;
  if v_rel.minor_id = v_me then
    raise exception 'You cannot be your own guardian' using errcode = '22023';
  end if;
  if (select is_minor from public.profiles where id = v_me) then
    raise exception 'A guardian account must be 18 or older' using errcode = '42501';
  end if;

  update public.guardian_relationships
     set guardian_id = v_me, status = 'active',
         linked_at = now(), relationship = nullif(trim(p_relationship), ''),
         invite_code = null
   where id = v_rel.id
  ...
  update public.ride_requests
     set guardian_id = v_me, guardian_status = 'pending'
...
begin
  ...
  if v_rel.guardian_id <> auth.uid() and not public.is_admin() then
    raise exception 'Only the guardian can remove this link' using errcode = '42501';
  end if;
...
begin
  ...
  if not public.is_guardian_of(v_req.rider_id, v_me) and not public.is_admin() then
    raise exception 'You are not the linked guardian for this rider' using errcode = '42501';
  end if;
  ...
  if p_approve then
    perform public.notify_user(v_ride.driver_id, 'guardian_approved',
      'Guardian approved a rider',
      'A guardian approved a rider for your ride to ' || v_ride.destination_label || '. You can accept them now.',
      v_ride.id, v_req.id);
    perform public.notify_user(v_req.rider_id, 'guardian_approved',
      'Your guardian approved this ride',
      'Waiting for the driver to confirm your seat.', v_ride.id, v_req.id);
  else
    perform public.notify_user(v_req.rider_id, 'guardian_denied',
      'Your guardian declined this ride',
...
begin
  ...
  if p_stars is null or p_stars < 1 or p_stars > 5 then
    raise exception 'Rating must be between 1 and 5 stars' using errcode = '23514';
  end if;
  ...
  if v_ride.status <> 'completed' then
    raise exception 'You can rate people once the ride is marked completed' using errcode = '22023';
  end if;
  ...
  if not v_ok then
    raise exception 'You can only rate people you actually shared this ride with' using errcode = '42501';
  end if;
...
declare
  v_me    uuid := auth.uid();
  v_group public.trusted_groups;
begin
  if v_me is null then
    raise exception 'You must be signed in' using errcode = '42501';
  end if;

  select * into v_group from public.trusted_groups
   where upper(join_code) = upper(trim(p_code));
  if not found then
    raise exception 'No trusted group matches that code' using errcode = 'P0002';
  end if;

  insert into public.group_members (group_id, user_id, status)
  values (v_group.id, v_me, case when v_group.is_open then 'active' else 'pending' end::public.member_status)
...
begin
  ...
  for r in select user_id from public.ride_participants where ride_id = p_ride and status = 'joined' loop
    perform public.notify_user(r.user_id, 'ride_removed', 'Ride removed',
      'A ride you joined to ' || coalesce(v_dest, 'a destination') || ' was removed by moderation.', p_ride);
  end loop;
end $$;
