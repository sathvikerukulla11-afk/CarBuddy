...
create policy profiles_private_select on public.profiles_private for select to authenticated
...
create policy profiles_private_insert on public.profiles_private for insert to authenticated
  with check (id = (select auth.uid()));

drop policy if exists profiles_private_update on public.profiles_private;
create policy profiles_private_update on public.profiles_private for update to authenticated
  using (id = (select auth.uid()) or public.is_admin())
  with check (id = (select auth.uid()) or public.is_admin());

-- -------------------------------------------------------- trusted groups ---
drop policy if exists groups_select on public.trusted_groups;
create policy groups_select on public.trusted_groups for select to authenticated using (true);

drop policy if exists groups_insert on public.trusted_groups;
create policy groups_insert on public.trusted_groups for insert to authenticated
...
create policy groups_update on public.trusted_groups for update to authenticated
...
create policy groups_delete on public.trusted_groups for delete to authenticated
...
create policy group_members_select on public.group_members for select to authenticated
  using (user_id = (select auth.uid()) or public.is_group_member(group_id)
         or public.is_group_admin(group_id) or public.is_admin());
...
create policy blocks_select on public.blocked_users for select to authenticated
  using (blocker_id = (select auth.uid()) or public.is_admin());

drop policy if exists blocks_insert on public.blocked_users;
create policy blocks_insert on public.blocked_users for insert to authenticated
  with check (blocker_id = (select auth.uid()));

drop policy if exists blocks_delete on public.blocked_users;
create policy blocks_delete on public.blocked_users for delete to authenticated
  using (blocker_id = (select auth.uid()));

-- ------------------------------------------------ guardian relationships ---
drop policy if exists guardian_select on public.guardian_relationships;
create policy guardian_select on public.guardian_relationships for select to authenticated
  using (guardian_id = (select auth.uid()) or minor_id = (select auth.uid()) or public.is_admin());
...
create policy guardian_insert on public.guardian_relationships for insert to authenticated
  with check (minor_id = (select auth.uid()) and guardian_id is null and status = 'pending');

drop policy if exists guardian_update on public.guardian_relationships;
create policy guardian_update on public.guardian_relationships for update to authenticated
...
create policy guardian_delete on public.guardian_relationships for delete to authenticated
...
create policy rides_select on public.rides for select to authenticated
  using (
    ...
    or (
      ...
      and (visibility in ('verified', 'approval')
...
create policy meetups_select on public.ride_meetups for select to authenticated
  using (public.is_ride_driver(ride_id) or public.is_ride_participant(ride_id)
         or public.guards_someone_on_ride(ride_id) or public.is_admin());

drop policy if exists meetups_write on public.ride_meetups;
create policy meetups_write on public.ride_meetups for all to authenticated
...
create policy requests_select on public.ride_requests for select to authenticated
  using (rider_id = (select auth.uid()) or public.is_ride_driver(ride_id)
         or public.is_guardian_of(rider_id) or public.is_admin());

-- ----------------------------------------------------- ride participants ---
drop policy if exists participants_select on public.ride_participants;
create policy participants_select on public.ride_participants for select to authenticated
  using (user_id = (select auth.uid()) or public.is_ride_driver(ride_id)
         or public.is_ride_participant(ride_id) or public.is_guardian_of(user_id)
         or public.is_admin());
