...
begin
  ...
  if v_ride.driver_id = v_me then
    raise exception 'You are the driver of this ride' using errcode = '22023';
  end if;
  if v_ride.status <> 'upcoming' then
    raise exception 'This ride is no longer accepting riders' using errcode = '22023';
  end if;
  if v_ride.depart_at < now() then
    raise exception 'This ride has already departed' using errcode = '22023';
  end if;
  ...
  if public.is_blocked_between(v_me, v_ride.driver_id) then
    raise exception 'This ride is not available to you' using errcode = '42501';
  end if;
  if v_ride.visibility = 'group' and not public.is_group_member(v_ride.group_id, v_me) then
    raise exception 'This ride is limited to members of a trusted group' using errcode = '42501';
  end if;
  if p_seats is null or p_seats < 1 or p_seats > 4 then
    raise exception 'Request between 1 and 4 seats' using errcode = '23514';
  end if;
  if v_ride.seats_remaining < p_seats then
    raise exception 'Only % seat(s) left on this ride', v_ride.seats_remaining using errcode = '23514';
  end if;

  select p.is_minor, p.full_name into v_minor, v_name
  ...
  if v_minor then
    select gr.guardian_id into v_guardian
     ...
     order by gr.linked_at nulls last
     limit 1;
  end if;
  ...
  begin
    ...
    values
      (p_ride_id, v_me, p_seats, nullif(trim(p_message), ''),
  ...
  if v_guardian is not null then
    perform public.notify_user(
      ...
      v_name || ' requested a ride to ' || v_ride.destination_label || '. Review it before the driver can accept.',
...
begin
  ...
  if v_ride.driver_id <> v_me and not public.is_admin() then
    raise exception 'Only the driver can answer this request' using errcode = '42501';
  end if;
  ...
  if p_accept then
    if v_ride.status <> 'upcoming' then
      raise exception 'This ride is no longer active' using errcode = '22023';
    end if;
    if v_req.guardian_status = 'pending' then
      raise exception 'This rider is waiting on parent/guardian approval' using errcode = '22023';
    end if;
    if v_req.guardian_status = 'denied' then
      raise exception 'This rider''s guardian declined the ride' using errcode = '22023';
    end if;
    if v_ride.seats_remaining < v_req.seats_requested then
      raise exception 'Only % seat(s) remain — you cannot accept a request for %.',
        v_ride.seats_remaining, v_req.seats_requested using errcode = '23514';
    end if;

    update public.rides
       set seats_taken = seats_taken + v_req.seats_requested
     where id = v_ride.id;
    ...
    perform public.notify_user(
      v_req.rider_id, 'request_accepted', 'You''re in!',
      'Your seat on the ride to ' || v_ride.destination_label || ' was confirmed.',
      v_ride.id, v_req.id);

    if v_req.guardian_id is not null then
      perform public.notify_user(
        v_req.guardian_id, 'ride_confirmed', 'Ride confirmed',
        'A ride you approved has been confirmed by the driver.',
        v_ride.id, v_req.id);
    end if;
  else
    update public.ride_requests
    ...
    perform public.notify_user(
      v_req.rider_id, 'request_rejected', 'Request declined',
      'Your request for the ride to ' || v_ride.destination_label || ' was declined. The seat is still open for others.',
      v_ride.id, v_req.id);
  end if;
...
begin
  ...
  if v_req.status = 'accepted' then
    ...
    update public.rides
       set seats_taken = greatest(0, seats_taken - v_req.seats_requested)
     where id = v_ride.id;

    perform public.notify_user(
      v_ride.driver_id, 'rider_left', 'A rider dropped out',
...
begin
  ...
  if v_ride.driver_id <> auth.uid() and not public.is_admin() then
    raise exception 'Only the driver can remove a rider' using errcode = '42501';
  end if;
  ...
  perform public.notify_user(
    p_user_id, 'removed_from_ride', 'Removed from a ride',
...
begin
  ...
  if v_ride.driver_id <> auth.uid() and not public.is_admin() then
    raise exception 'Only the driver can cancel this ride' using errcode = '42501';
  end if;
  ...
  for r in select user_id from public.ride_participants
    ...
    perform public.notify_user(
      r.user_id, 'ride_cancelled', 'Ride cancelled',
...
begin
  ...
  if v_ride.driver_id <> auth.uid() and not public.is_admin() then
    raise exception 'Only the driver can complete this ride' using errcode = '42501';
  end if;
  ...
  for r in select user_id from public.ride_participants
    ...
    perform public.notify_user(
      r.user_id, 'ride_completed', 'How was your ride?',
